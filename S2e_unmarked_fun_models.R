# N-mixture and occuRN functions
 library(matrixcalc)
 library(unmarked)

pcountPoisPois <- function(formula, data, K, mixture = c("P", "NB", "ZIP"), starts,
                   method = "BFGS", se = TRUE,
                   engine = c("C", "R"), trace = 0, ...)
{
	...
	#Changed : plogis by exp for "p.ij"
     p.ij <- exp(V %*% parms[(nAP + 1) : (nAP + nDP)] + V.offset)
            theta.ik <- rep(theta.i, each = K + 1)
            p.ijk <- rep(p.ij, each = K + 1)
			
	#Changed : dbinom by dpois : parameter becomes (k*p)		
     bin.ijk <- dpois(y.ijk, (k.ijk*p.ijk))
    ...
    fm <- optim(starts, nll, method=method, hessian=se, control=list(trace=trace)) #, ...)
    ...
    if( is.singular.matrix(fm$hessian) ) covMat <- matrix(NA, nP, nP)  else  covMat <- invertHessian(fm, nP, se)
